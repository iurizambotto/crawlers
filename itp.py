...
class ITP:
    ...
    def load_schedule_list(self, class_name):
        ...
        entities = [ITPEntity(**value) for value in all_values]
    ...
    def get_table_by_question(self, question):
        ...
        values = [
            [value.text for value in line.find_elements(By.TAG_NAME, "td")]
            for line in lines
        ][1:]
        question.click()
        return values
            ...
            if column_type == "datetime":
                if getattr(self, column) != "":
                    setattr(
                        self,
                        f"{column}_dtobj",
                        datetime.strptime(getattr(self, column), str_pattern),
                    )
                else:
